...
# 🚀 Redis Cache Test Script สำหรับ ShopImage API
# ทดสอบ cache functionality ของ /api/convert endpoint

echo "🔗 Testing Redis Cache for ShopImage API"
echo "========================================"
...
echo "📤 Test 1: Upload image (should be cache miss)"
echo "   Command: curl -X POST http://localhost:8080/api/convert -F \"file=@test_image.jpg\" -F \"target_w=800\" -F \"target_h=600\" -F \"format=webp\""
echo ""

# ทดสอบครั้งที่ 1 (ควรเป็น cache miss)
echo "🔄 First request (Cache Miss expected)..."
RESPONSE1=$(curl -s -X POST http://localhost:8080/api/convert \
...
RESPONSE2=$(curl -s -X POST http://localhost:8080/api/convert \
  -F "file=@test_image.jpg" \
  -F "target_w=800" \
  -F "target_h=600" \
  -F "format=webp")
...
if [ "$URL1" != "null" ] && [ "$URL2" != "null" ]; then
    echo "✅ Both requests successful"
    echo "   First URL:  $URL1"
    echo "   Second URL: $URL2"
    
    # ตรวจสอบว่าเป็น cache hit หรือไม่ (URL ควรต่างกันเพราะเป็นไฟล์ใหม่)
    if [ "$URL1" != "$URL2" ]; then
    ...
    echo "❌ One or both requests failed"
fi

echo ""
echo "📋 Server Logs (check terminal for cache messages):"
echo "   Look for: 💭 Cache miss, 🎯 Cache hit, 💾 Cached result"
echo ""
...
echo "   - First request should show '💭 Cache miss'"
echo "   - Second request should show '🎯 Cache hit'"
echo "   - Both should show '💾 Cached result'"
echo ""
